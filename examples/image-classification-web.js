 ...
 *     document.querySelector('url-hash-router').currentRoute.path
 *
 ...
 * was needed then logic using `(window.app !== undefined)` would not be
 * needed and <script type="module"> with modern JS could be used.
 *
 * See similar code for DataFormsJS Framework using Handlebars and Vue
 * in the file [image-classification.js] Much of this code is copied
 * and modified from that file.
 */
...
(function() {
    ...
    /**
     * Templating function (assigned to `window` so it can be in global scope)
    ...
    /**
     * Get path of the current route
     */
    function getPagePath() {
        if (window.app !== undefined) {
            // DataFormsJS Framework using [polyfill.js]
            return app.activeController.path;
        } else {
            return document.querySelector('url-hash-router').currentRoute.path;
        }
    }
    ...
    /**
     * Setup page after <json-data> downloads images or renders them from cache
     */
